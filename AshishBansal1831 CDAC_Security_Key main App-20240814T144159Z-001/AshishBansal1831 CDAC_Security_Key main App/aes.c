...
int main(int argc, char *argv[]) 
{
    ...
    User_ID_Label = gtk_entry_new(); // Create an entry widget for username
    gtk_box_pack_start(GTK_BOX(login_page), User_ID_Label, FALSE, FALSE, 5); // Add entry to vertical box
...
static void on_login_button_clicked(GtkButton *button, gpointer user_data) {
    const gchar *username = gtk_entry_get_text(GTK_ENTRY(User_ID_Label)); // Get username from entry
...
// gboolean SignUpDialogBox(gpointer dialog) {
//     printf("Did not get usb\n");
//     if (initUSB()==EXIT_SUCCESS) {
